...
if [ "${single_gnn_training}" = True ]; then
    ...
    echo $config_path
    bash ./host/script/run_train_single.sh "/DATA_IN/${env_in_data_filename}" "/GNN_TMP" "/DATA_OUT" ${graph_CSVDataset_name} "${config_path}" "/MODELS" "/DATA_IN/card_transaction.v1.csv"
fi;
