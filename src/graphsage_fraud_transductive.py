...
class Model(nn.Module):
    def __init__(self, vocab_size, hid_size):
        super().__init__()
        ...
        self.encoder = GraphSAGE(hid_size, hid_size, hid_size, 1, F.relu, "mean")
...
def train(args, device, g, train_dataloader, val_dataloader, test_dataloader, model):
    best_rocauc = 0
    best_model_path = args.out_model_file
    eval_every = 3
    lrate=0.0005
    ...
    with train_dataloader.enable_cpu_affinity():
        for epoch in range(30):
            ...
            print("Epoch {:05d} | Train Loss {:.4f} | Valid Loss {:.4f}".format(epoch, total_loss / (it + 1) ,total_val_loss / (it2 + 1)))

            if epoch % eval_every == 0 and epoch != 0:
                epoch_time = time.time()
                rocauc = evaluate(model, test_dataloader)
                print(
                    "Epoch {:05d} | Epoch time {:.4f} | Test roc_auc_score {:.4f}".format(
                        epoch, epoch_time - start, rocauc
                    )
                )
...
def main(arglist):
    ...
    parser.add_argument(
        "--CSVDataset",
        default="/GNN_TMP/sym_tabformer_hetero_CSVDataset_FINAL/",
        help="Path to CSVDataset",
    )
    parser.add_argument(
        "--out_model_file",
        default="/GNN_TMP/sym_tabformer_hetero_CSVDataset_FINAL/tabformer_graphsage_2L_64_emb.pt",
    ...
    print(hg)
    train_mask = hg.edges["transaction"].data["train_mask"]
    val_mask = hg.edges["transaction"].data["val_mask"]
    test_mask = hg.edges["transaction"].data["test_mask"]
    ...
    model = Model(vocab_size, hid_size).to(device)
...
#    model.load_state_dict(torch.load(args.out_model_file))
#    node_emb = model.emb.weight.data
#    torch.save(node_emb, args.out_emb_file)

    print("Inference...")
    model.eval()
    model.load_state_dict(torch.load(args.out_model_file))
    infer_batch_size=2048
    node_emb = model.inference(g, device, infer_batch_size)
    print("node emb shape: ", node_emb.shape)
    torch.save(node_emb, args.out_emb_file)

    # roc_auc on the different splits after training completion
    rocauc_val = evaluate(model, val_dataloader)
    print("val rocauc", rocauc_val)
    rocauc_test = evaluate(model, test_dataloader)
    print("test rocauc", rocauc_test)
    rocauc_train = evaluate(model, train_dataloader)
    print("train rocauc", rocauc_train)
