...
class Model(nn.Module):
    def __init__(self, vocab_size, hid_size):
        ...
        self.encoder = DistSAGE(hid_size, hid_size, hid_size, 1, F.relu, "mean")
...
def run(args, device, data):
    ...
    model = Model(g.num_nodes(), args.num_hidden).to(device)
    ...
    model_noDDP = Model(g.num_nodes(), args.num_hidden).to(device)
    ...
    for epoch in range(args.num_epochs):
        ...
        if g.rank() == 0:
            ...
            print(
                "Epoch {:05d} | roc_auc {:.4f} | accuracy {:.4f} ".format(
                    epoch, rocauc, ap_score
                )
            )
    ...
    if not args.standalone:
        ...
        with th.no_grad():
            ...
            print(x.shape)
            node_emb = model.module.inference(g, x, infer_batch_size, device)
        if g.rank() == 0:
...
def main(args):
    ...
    print("rank:", g.rank())
    print("number of edges", g.num_edges())  # full graph
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_out",
        type=str,
        help="output for model /your_path/tabformer_graphsage_2L_64.pt",
    )
    parser.add_argument(
        "--nemb_out",
        type=str,
