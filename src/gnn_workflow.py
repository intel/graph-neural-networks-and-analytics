...
def main(args):
    ...
    emb_file = os.path.join(CSVDataset_dir, "node_emb.pt")
    model_file = os.path.join(CSVDataset_dir, "tabformer_graphsage_2L_64_emb.pt")
...
- file_name: nodes_1.csv
  ntype: merchIdx
    ...
    # 5 Train model, save model and embeddings, evaluate roc_auc on test set
    graphsage_fraud_transductive.main(
        [
            "--mode",
            "cpu",
            "--CSVDataset",
    ...
    t_train = time.time()
    print("time to train GNN: ", t_train - t_csvDataset)
    ...
    print("Time to append node embeddings to edge features CSV", time.time() - t_train)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="GNN_Workflow")
    parser.add_argument(
        "--data_in",
        type=file,
    ...
    parser.add_argument(
        "--data_out",
        default="/DATA_OUT/tabformer_with_gnn_emb.csv",
        help="Output file with path (tabformer_with_gnn_emb.csv)",
    )
    parser.add_argument(
        "--gnn_tmp",
        default="/GNN_TMP/",
        help="Path to gnn_tmp dir",
    )
